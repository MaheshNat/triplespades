...
var AppComponent = /** @class */ (function () {
    function AppComponent(authService, gameService, socketService) {
    ...
    AppComponent.prototype.ngOnInit = function () {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function () {
            ...
            return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                ...
                context = this;
                window.addEventListener('beforeunload', function (e) { return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](_this, void 0, void 0, function () {
                    return tslib__WEBPACK_IMPORTED_MODULE_0__["__generator"](this, function (_a) {
                        switch (_a.label) {
                            case 0: return [4 /*yield*/, context.socketService.emit('leave', context.user.name)];
                            case 1:
                                _a.sent();
                                return [2 /*return*/];
                        }
...
var AuthComponent = /** @class */ (function () {
    ...
    AuthComponent.prototype.onSubmit = function (form) {
        var _this = this;
        this.isLoading = true;
        this.authService.login(form.value.email, form.value.password).subscribe(function (resData) {
            console.log(resData);
...
var AuthService = /** @class */ (function () {
    function AuthService(http, router, socketService) {
        this.http = http;
        this.router = router;
        this.socketService = socketService;
    ...
    AuthService.prototype.register = function (name, email, password) {
        ...
        return this.http
            ...
            var expirationDate = new Date(new Date().getTime() + _this.EXPIRES_IN * 1000);
            var user = new _user_model__WEBPACK_IMPORTED_MODULE_1__["User"](resData.name, email, resData.token, expirationDate, true);
            _this.user.next(user);
            _this.autoLogout(_this.EXPIRES_IN * 1000);
            localStorage.setItem('userData', JSON.stringify(user));
            _this.socketService.emit('join', user.name);
    ...
    AuthService.prototype.autoLogin = function () {
        ...
        if (!userData)
            return;
        var user = new _user_model__WEBPACK_IMPORTED_MODULE_1__["User"](userData.name, userData.email, userData._token, new Date(userData._tokenExpirationDate), true);
        if (user.token) {
            this.user.next(user);
            var expirationDate = new Date(userData._tokenExpirationDate).getTime() -
                new Date().getTime();
    ...
    AuthService.prototype.logout = function () {
        ...
        return this.http
            ...
            .pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["catchError"])(this.handleError), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_4__["tap"])(function (message) {
            var expirationDate = new Date(new Date().getTime() + _this.EXPIRES_IN * 1000);
            var user = new _user_model__WEBPACK_IMPORTED_MODULE_1__["User"](_this.user.value.name, _this.user.value.email, null, expirationDate, false);
            _this.socketService.emit('leave', user.name);
    ...
    AuthService.prototype.autoLogout = function (expiresIn) {
        var _this = this;
...
var User = /** @class */ (function () {
    function User(name, email, _token, _tokenExpirationDate, authenticated) {
        this.name = name;
        this.email = email;
        this._token = _token;
...
var Card = /** @class */ (function () {
    function Card(suit, name, rank, pointValue, image) {
        this.suit = suit;
        this.name = name;
        this.rank = rank;
...
var GameComponent = /** @class */ (function () {
    function GameComponent(socketService, authService, gameService, modalService) {
        this.socketService = socketService;
        this.authService = authService;
        this.gameService = gameService;
    ...
    GameComponent.prototype.open = function (content) {
    ...
    GameComponent.prototype.ngOnInit = function () {
        ...
        var _this = this;
        try {
            for (var _b = tslib__WEBPACK_IMPORTED_MODULE_0__["__values"](['AC', 'AD', 'AH', 'AS']), _c = _b.next(); !_c.done; _c = _b.next()) {
                var suit = _c.value;
        ...
        this.gameService.game.subscribe(function (game) {
            _this.game = game;
        });
    };
    GameComponent.prototype.onBid = function () {
    ...
    GameComponent.prototype.incrementBidValue = function () {
        this.bidValue += this.bidValue === 250 ? 0 : 5;
    };
    GameComponent.prototype.decrementBidValue = function () {
        this.bidValue -= this.bidValue === 125 ? 0 : 5;
    };
    GameComponent.prototype.getImage = function (card) {
    ...
    GameComponent.prototype.playCard = function (card) {
        ...
        if (this.user.name === this.game.players[this.game.turn].name) {
            if (this.game.hand.length > 0 &&
                ...
                this.game.cards.findIndex(function (card) { return card.suit === _this.game.hand[0].suit; }) !== -1 &&
                ...
                this.game.cards.splice(this.game.cards.findIndex(function (_card) { return _card === card; }), 1);
...
var Game = /** @class */ (function () {
    function Game(highestBidPlayer, started, gameMode, cards, players, startTime, turn, hand, collectedCards, trumpSuit, partnerCard, partner, score, won, time, endTime) {
        if (hand === void 0) { hand = []; }
        if (collectedCards === void 0) { collectedCards = []; }
...
var GameService = /** @class */ (function () {
    function GameService(socketService, router, http, authService) {
        var _this = this;
        this.socketService = socketService;
        this.router = router;
        this.http = http;
        ...
        this.http.get(_environments_environment__WEBPACK_IMPORTED_MODULE_9__["environment"].apiUrl + "/cards").subscribe(function (cards) {
            _this.allCards = cards;
        });
        this.game.subscribe(function (game) { return (_this.gameValue = game); });
        this.socketService.listen('bid').subscribe(function (data) {
            _this.gameValue.highestBidPlayer = new _shared_player_model__WEBPACK_IMPORTED_MODULE_5__["Player"](data.name, data.bid, true);
            _this.update();
        });
        this.socketService.listen('start_selection').subscribe(function (data) {
            clearInterval(_this.timeInterval);
            clearTimeout(_this.selectionStartTimeout);
            _this.gameValue.gameMode = _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].TRUMP_SELECTION;
            _this.update();
            if (_this.user.name === _this.gameValue.highestBidPlayer.name) {
                _this.trumpSelectionTimeout = setTimeout(function () {
                    clearInterval(_this.timeInterval);
                    _this.gameValue.trumpSuit = ['S', 'C', 'H', 'D'][Math.floor(Math.random() * 4)];
                    _this.socketService.emit('trump_selection', _this.gameValue.trumpSuit);
                    _this.gameValue.gameMode = _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].PARTNER_SELECTION;
                    _this.update();
                }, _this.TRUMP_SELECTION_TIME);
            }
        });
        this.socketService
            .listen('trump_selection')
            .subscribe(function (trumpSuit) {
            _this.gameValue.trumpSuit = trumpSuit;
            _this.gameValue.gameMode = _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].PARTNER_SELECTION;
            _this.update();
            if (_this.user.name === _this.gameValue.highestBidPlayer.name) {
                _this.partnerCardSelectionTimeout = setTimeout(function () {
                    clearInterval(_this.timeInterval);
                    _this.gameValue.partnerCard = _this.allCards[Math.floor(Math.random() * _this.allCards.length)];
                    _this.socketService.emit('partner_selection', _this.gameValue.partnerCard);
                    _this.update();
                }, _this.PARTNER_CARD_SELECTION_TIME);
            }
        });
        this.socketService
            .listen('partner_selection')
            .subscribe(function (partnerCard) {
            _this.gameValue.partnerCard = partnerCard;
            _this.gameValue.partner = _this.gameValue.cards
                .map(function (card) { return card.rank + card.suit; })
                .includes(_this.gameValue.partnerCard.rank + _this.gameValue.partnerCard.suit);
            _this.update();
        });
        this.socketService.listen('start_playing').subscribe(function (turn) {
            _this.gameValue.turn = turn;
            _this.gameValue.gameMode = _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].PLAYING;
            _this.update();
        });
        this.socketService.listen('play_card').subscribe(function (card) {
            _this.gameValue.hand.push(card);
            _this.gameValue.turn =
                (_this.gameValue.turn + 1) % _this.gameValue.players.length;
            _this.update();
        });
        this.socketService.listen('hand_end').subscribe(function (card) {
            _this.gameValue.hand.push(card);
            _this.gameValue.turn = _this.getHandWinner();
            console.log(_this.gameValue.turn);
            if (_this.user.name === _this.gameValue.players[_this.gameValue.turn].name) {
                _this.gameValue.collectedCards = _this.gameValue.collectedCards.concat(_this.gameValue.hand);
            }
            _this.gameValue.hand = [];
            _this.update();
        });
        this.socketService.listen('playing_end').subscribe(function () {
            var score = _this.gameValue.collectedCards.reduce(function (acc, card) { return acc + card['point_value']; }, 0);
            console.log(_this.gameValue.partner);
            if (_this.user.name === _this.gameValue.highestBidPlayer.name) {
                if (_this.gameValue.partner) {
                    _this.socketService.emit('score_final', score * 2);
                    console.log(1);
                }
                _this.socketService.emit('score', score);
                console.log(2);
            }
            else if (_this.gameValue.partner) {
                _this.socketService.emit('score', score);
                console.log(3);
            }
        });
        this.socketService.listen('score_final').subscribe(function (score) {
            _this.gameValue.gameMode = _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].END;
            _this.gameValue.score = score;
            _this.gameValue.endTime = new Date();
            if (_this.gameValue.score >= _this.gameValue.highestBidPlayer.bid)
                _this.gameValue.won = true;
            if (_this.gameValue.partner) {
                _this.http
                    .post(_environments_environment__WEBPACK_IMPORTED_MODULE_9__["environment"].apiUrl + "/game", {
                    start_time: _this.gameValue.startTime,
                    end_time: _this.gameValue.endTime,
                    max_bid: _this.gameValue.highestBidPlayer.bid,
                    bidder_name: _this.gameValue.highestBidPlayer.name,
                    trump_suit: _this.getSuit(_this.gameValue.trumpSuit),
                    partner_card: _this.gameValue.partnerCard.name + " of " + _this.gameValue.partnerCard.suit,
                    score: _this.gameValue.score,
                })
                    .subscribe(function (data) { return console.log(data); });
            }
            _this.gameValue.started = false;
            _this.update();
        });
        this.socketService.listen('end_game').subscribe(function () {
            _this.game.next(new _game_model__WEBPACK_IMPORTED_MODULE_3__["Game"](null, false));
            clearInterval(_this.timeInterval);
            clearTimeout(_this.selectionStartTimeout);
            _this.router.navigate(['/']);
        });
    }
    GameService.prototype.startGame = function (defaultBidder, cards, players, startTime) {
        var _this = this;
        var game = new _game_model__WEBPACK_IMPORTED_MODULE_3__["Game"](new _shared_player_model__WEBPACK_IMPORTED_MODULE_5__["Player"](defaultBidder, 125, true), true, _game_model__WEBPACK_IMPORTED_MODULE_3__["GameMode"].BIDDING, cards, players, startTime);
        this.game.next(game);
        this.timeInterval = setInterval(function () {
            if (game.time > 0)
                game.time--;
            _this.game.next(game);
        }, 1000);
        this.selectionStartTimeout = setTimeout(function () {
            _this.socketService.emit('start_selection', null);
        }, this.BIDDING_TIME);
    };
    GameService.prototype.onTrumpSuitSelection = function (trumpSuit) {
    ...
    GameService.prototype.onPartnerCardSelection = function (partnerCard) {
        var _this = this;
        ...
        this.socketService.emit('start_playing', this.gameValue.players.findIndex(function (player) { return player.name === _this.gameValue.highestBidPlayer.name; }));
    };
    GameService.prototype.getHandWinner = function () {
        var shiftedPlayers = this.gameValue.players.slice();
        ...
        var winner = 0;
        var winnerCard = this.gameValue.hand[0];
        for (var i = 1; i < this.gameValue.hand.length; i++) {
            var card = this.gameValue.hand[i];
        ...
        return this.gameValue.players.findIndex(function (player) { return player === shiftedPlayers[winner]; });
    };
    GameService.prototype.getSuit = function (suit) {
        switch (suit) {
            case 'H':
                return 'Hearts';
            case 'S':
...
var HeaderComponent = /** @class */ (function () {
    function HeaderComponent(authService, gameService, router) {
        this.authService = authService;
        this.gameService = gameService;
        this.router = router;
    ...
    HeaderComponent.prototype.ngOnInit = function () {
        ...
        this.gameSub = this.gameService.game.subscribe(function (game) {
            _this.game = game;
        });
    };
    ...
    HeaderComponent.prototype.onLogout = function () {
        var _this = this;
        this.authService.logout().subscribe(function (resData) {
            _this.router.navigate(['/auth']);
        });
    };
...
var HomeComponent = /** @class */ (function () {
    function HomeComponent(router, authService, gameService) {
        this.router = router;
        this.authService = authService;
        this.gameService = gameService;
        this.isAuthenticated = false;
    ...
    HomeComponent.prototype.onNavigate = function () {
...
var ScoreboardComponent = /** @class */ (function () {
    function ScoreboardComponent(http) {
        this.http = http;
    }
    ScoreboardComponent.prototype.ngOnInit = function () {
        ...
        this.http
            ...
            .subscribe(function (scoreboard) {
            _this.scoreboard = scoreboard.map(function (score) {
...
var Player = /** @class */ (function () {
    function Player(name, bid, highestBidder) {
        this.name = name;
        this.bid = bid;
...
var WaitingRoomComponent = /** @class */ (function () {
    function WaitingRoomComponent(socketService, gameService, authService, http, router) {
        this.socketService = socketService;
        this.gameService = gameService;
        this.authService = authService;
        this.http = http;
    ...
    WaitingRoomComponent.prototype.ngOnInit = function () {
        ...
        this.http
            ...
            _this.players = players.map(function (player) { return new _shared_player_model__WEBPACK_IMPORTED_MODULE_4__["Player"](player); });
        });
        this.authService.user.subscribe(function (user) {
            _this.user = user;
        });
        this.socketService.emit('join', this.user.name);
        this.socketService.listen('join').subscribe(function (playerName) {
            console.log(playerName + " joined");
            _this.players.push(new _shared_player_model__WEBPACK_IMPORTED_MODULE_4__["Player"](playerName));
        });
        this.socketService.listen('leave').subscribe(function (playerName) {
            console.log(playerName + " left");
            _this.players.splice(_this.players.findIndex(function (_player) { return _player.name === playerName; }), 1);
        });
        this.socketService
            .listen('start_game')
            .subscribe(function (data) {
            _this.gameService.startGame(data['default_bidder'], data['cards'][_this.players.findIndex(function (player) { return player.name === _this.user.name; })], _this.players, new Date());
            _this.router.navigate(['/game']);
        });
    };
    WaitingRoomComponent.prototype.ngOnDestroy = function () { };
    WaitingRoomComponent.ctorParameters = function () { return [
        { type: _socket_service__WEBPACK_IMPORTED_MODULE_2__["SocketService"] },
